...
jar(){
  ...
  clojure \
    ...
    :main-ns Agent-K.main \
    :filename "\"out/Agent-K-$COMMIT_COUNT-$COMMIT_HASH-$OPERATING_SYSTEM.jar\"" \
...
ui_install(){
  ...
  mkdir -p out/ui/
  cp src/Agent_K/index.html out/ui/index.html
  cp src/Agent_K/style.css out/ui/style.css
}
